...
--------------------------------------CHECK PD----------------------------------------
    ...
    if checkPD == 0 then
        TriggerClientEvent("vorp:TipRight", _source, 'Neni dostatek sheriffu', 6000)
 ...
 --------------------------------------------------------------------------------  
        ...
        if sellFcked == 1 then
            if _x == 0 then
                TriggerClientEvent("vorp:TipRight", _source, 'Nemas nic na prodej', 6000)    
            
            
            else 
                print(json.encode(luck))
                if luck > 4 then
                    TriggerClientEvent("vorp:TipRight", _source, 'Nechci tvuj tovar idiote', 6000)
                else
                    ...
                    if count >= AmountToSell then
                        ...
                        local text = AmountToSell.. ' ' ..randomitem.. ' za ' ..price..' dolaru ' 
                        Character.addCurrency(0 , price)
                        TriggerClientEvent("vorp:TipRight", _source, 'Prodal jsi '..AmountToSell.. ' ' ..randomitem.. ' za ' ..price..' dolaru ', 6000)
                        local webhook = Config.WebHook
  	                    SendWebhook(webhook, _source, text)
                    else
                        TriggerClientEvent("vorp:TipRight", _source, 'Chci '..AmountToSell..' '..randomitem..' a ty tolik nemas!', 6000) 
                    end
                end
            end
        else
            TriggerClientEvent("vorp:TipRight", _source, 'Jsi moc daleko', 6000)
        end
    end
end)
